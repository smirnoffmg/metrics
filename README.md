...
**Can you trust that date?** The same forecast, replayed from every Monday of two years of Hibernate with only what Jira showed that day. First it picks how many recent weeks to draw from: over 8 weeks, forecasts from the last 26 weeks scored a 12% lower CRPS than from the last 12, and the last 52 were no better than 26, so the report forecasts from 26. The backlog forecast promises 30 weeks, but 30-week forecasts made a week apart share 29 weeks of outcome, so two years hold just 3 independent ones - too few to judge. The report judges the longest horizon with enough evidence instead: over 8 weeks, 83 of 90 "85% chance" promises came true (12 independent outcomes), cautious rather than optimistic. Nor would correcting forecasts by their past errors help: their errors drift over time (y-plot p < 0.001 over 4 weeks), since Hibernate's pace changes. Kafka, whose throughput halved over the same two years, forecasts best when recent weeks weigh most (a half-life of 4 weeks, 8% lower CRPS over 8 weeks than the last 12 weeks), which also leaves no sign of drift in its errors - yet only 55% of its 8-week promises come true, and the report flags that in red: a steady bias, too narrow more than too high. Recalibrated by their past errors, its 8-week forecasts keep 78% of their promises, yet score a worse CRPS, so the report leaves the forecast uncorrected.
...
- **Can you trust the forecast?** The report replays it: from every past Monday it forecasts, from only what Jira showed then, how many issues the team would finish, and checks what actually happened - over 4 and 8 weeks and over the forecast's own horizon. "85% forecasts held" says how often the 85% promise came true; a u-plot per horizon shows whether past forecasts leaned optimistic, pessimistic or overconfident. Forecasts a week apart share most of their outcome, so each horizon also counts its independent outcomes; the tile judges the longest horizon with at least 10, and with none it states the count instead of a verdict. Beneath the table the report says whether forecast errors keep a steady bias (a u-plot test) or drift over time (a y-plot test, after Brocklehurst and Littlewood), tested on independent outcomes only. Drifting errors mean the team's pace changes. Steady but biased ones get the book's remedy tried: every past forecast is recalibrated by the u-plot of the forecasts whose outcomes were known by its Monday, and when that scores a better CRPS the forecast itself is recalibrated and its tile says so. Better calibration alone is not enough - Fenton and Pfleeger also want the corrected forecasts to win on a global score. Status, resolution and assignee are rolled back through each issue's changelog, so reopened work counts as it stood then. It needs history: fetch a year or more (`resolved >= -365d OR resolution = Unresolved`).
- **Forecasts from the pace that predicts your team best.** The same replay picks which past weeks the forecast draws from - the last 12, 26 or 52 weeks equally, or every week weighted by recency with a half-life of 4 or 8 weeks - by the lowest CRPS, a score of how far past forecasts landed from what happened, at the longest short horizon with at least 10 independent outcomes. Paces within 5% of the best count as equal and the earlier in that list wins, so a year-old or a weighted pace needs a real edge; with too little history the forecast keeps the last 12 weeks. The report's table shows every pace's scores.
